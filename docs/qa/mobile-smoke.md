# Mobile landscape smoke checklist

Use this before claiming mobile polish.

- [ ] Open the Vite preview URL on a phone/tablet in landscape.
...
- [ ] Confirm no child-facing external links appear.

Automated local proxy for this slice: `npm run build` plus `npm run dev` / `npm run preview` HTTP 200. Full device testing remains a manual smoke pass.
